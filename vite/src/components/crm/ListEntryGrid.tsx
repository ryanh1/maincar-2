...
import type { GridCell, GridColumn, Rectangle, Theme } from '@glideapps/glide-data-grid'
...
export function ListEntryGrid({ attributes, entries, totalCount, hasNextPage, isFetchingNextPage, fetchNextPage }: ListEntryGridProps) {
  const { user } = useAuth()
  const colors = useGridColors()
  const columns = useMemo(() => attributes.filter((attribute) => !attribute.isArchived), [attributes])
  const gridColumns = useMemo<GridColumn[]>(() => columns.map((attribute, index) => ({ id: attribute.slug, title: attribute.name, width: index === 0 ? 220 : 160 })), [columns])
  const getCellContent = useCallback(([column, row]: readonly [number, number]): GridCell => {
    const entry = entries[row]
    const attribute = columns[column]
    if (!entry || !attribute) return { kind: GridCellKind.Loading, allowOverlay: false }
  ...
  if (gridColumns.length === 0) return <p className="text-sm text-muted-foreground">This list’s object has no fields yet.</p>
  return <div className="h-full min-h-0 border border-border bg-bg"><DataEditor columns={gridColumns} getCellContent={getCellContent} customRenderers={[chipCellRenderer]} rows={totalCount} freezeColumns={1} rowHeight={32} rowMarkers="none" verticalBorder smoothScrollX smoothScrollY onVisibleRegionChanged={onVisibleRegionChanged} theme={theme} width="100%" height="100%" /></div>
}
