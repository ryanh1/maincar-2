...
export function KeyboardProvider({ children }: KeyboardProviderProps) {
  ...
  const commands = useMemo<KeyboardCommand[]>(() => {
    ...
    const settings: KeyboardCommand[] = [
      { id: 'settings-profile', title: 'Profile', group: 'Settings', keywords: ['settings account'], execute: () => visit('/settings?tab=profile') },
    ]

    if (org) {
      settings.push(
        { id: 'settings-organization', title: 'Organization', group: 'Settings', keywords: ['settings'], execute: () => visit('/settings?tab=organization') },
        { id: 'settings-phone-numbers', title: 'Phone numbers', group: 'Settings', keywords: ['settings calling'], execute: () => visit('/settings?tab=numbers') },
        { id: 'settings-email-templates', title: 'Email templates', group: 'Settings', keywords: ['settings compose'], execute: () => visit('/settings?tab=email-templates') },
        { id: 'settings-integrations', title: 'Integrations', group: 'Settings', keywords: ['settings email'], execute: () => visit('/settings?tab=integrations') },
      )
    }

    if (org && isAdmin) {
      settings.push({ id: 'settings-members', title: 'Members', group: 'Settings', keywords: ['settings team'], execute: () => visit('/settings?tab=members') })
    }
