...
test('saves one visible numeric disposition in Chromium', async ({ page }) => {
...
test('searches and saves an unpinned disposition with 0 then Enter in Chromium', async ({ page }) => {
  const consoleErrors: string[] = []
  ...
  await page.route('**/api/orgs/org-fixture/dispositions', (route) =>
    route.fulfill({ json: { dispositions: [
      { id: 'connected', value: 'connected', label: 'Connected', color: 'option-1', icon: 'phone', category: 'connected', isStandard: true, isPinned: true, pinOrder: 0, sortOrder: 0, isArchived: false, createdAt: '', updatedAt: '' },
      { id: 'callback', value: 'callback', label: 'Call back', color: 'option-7', icon: 'calendar', category: 'connected', isStandard: false, isPinned: false, pinOrder: null, sortOrder: 7, isArchived: false, createdAt: '', updatedAt: '' },
    ] } }),
  )
  await page.route('**/api/orgs/org-fixture/calls/call-fixture/disposition', async (route) => {
  ...
  await page.goto('/__fixtures/dialer-disposition-bar')
  await expect(page.getByRole('button', { name: 'More call outcomes' })).toBeVisible()
  await page.keyboard.press('0')
  const search = page.getByPlaceholder('Search call outcomes')
  await expect(search).toBeVisible()
  await search.fill('back')
  await search.press('Enter')

  await expect.poll(() => savePayload).toEqual({ dispositionId: 'callback' })
  expect(consoleErrors).toEqual([])
})
