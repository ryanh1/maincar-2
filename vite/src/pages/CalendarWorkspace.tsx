...
import { useCreateCalendarEvent, useDeleteCalendarEvent, useGetCalendarEvents, useGetCalendarSources, useUpdateCalendarEvent, useUpdateCalendarSource } from '@/hooks/calendar'
import type { CalendarEvent, CalendarEventCreateInput, CalendarEventPatch } from '@/lib/calendarTypes'
...
export function CalendarWorkspace() {
  ...
  const saveEvent = (input: CalendarEventCreateInput | CalendarEventPatch, event: CalendarEvent | null) => {
    if (!org) return
    if (event) updateEvent.mutate({ orgId: org.id, eventId: event.id, expectedVersion: event.providerVersion ?? null, patch: input as CalendarEventPatch }, { onSuccess: () => { setEditor(null); setSelectedEvent(null); toast.success('Event updated.') }, onError: (error) => showError('update the event', error) })
  ...
  const removeEvent = (event: CalendarEvent) => {
    if (!org) return
    deleteEvent.mutate({ orgId: org.id, eventId: event.id, expectedVersion: event.providerVersion ?? null }, { onSuccess: () => { setSelectedEvent(null); toast.success('Event deleted.') }, onError: (error) => showError('delete the event', error) })
  }
  const moveEvent = (event: CalendarEvent, day: Date) => {
    ...
    updateEvent.mutate({ orgId: org.id, eventId: event.id, expectedVersion: event.providerVersion ?? null, patch: { time, timeZone: event.kind === 'all-day' ? null : event.timeZone ?? timeZone ?? null } }, { onSuccess: () => toast.success('Event moved.'), onError: (error) => showError('move the event', error) })
  }
  const resizeEvent = (event: CalendarEvent, durationMinutes: number) => {
    ...
    updateEvent.mutate({ orgId: org.id, eventId: event.id, expectedVersion: event.providerVersion ?? null, patch: { time: { kind: 'timed', startsAt: event.startsAt, endsAt }, timeZone: event.timeZone ?? timeZone ?? null } }, { onSuccess: () => toast.success('Event resized.'), onError: (error) => showError('resize the event', error) })
  }
