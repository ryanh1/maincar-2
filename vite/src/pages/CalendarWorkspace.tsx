...
function providerErrorMessage(error: Error): string {
...
export function CalendarWorkspace() {
  ...
  const saveEvent = (input: CalendarEventCreateInput | CalendarEventPatch, event: CalendarEvent | null, scope: CalendarRecurrenceScope) => {
    if (!org) return
    setEditorSaveError('')
    if (event) updateEvent.mutate({ orgId: org.id, eventId: event.id, expectedVersion: event.providerVersion ?? null, scope, patch: input as CalendarEventPatch }, { onSuccess: () => { setEditor(null); setSelectedEvent(null); toast.success('Event updated.') }, onError: (error) => { setEditorSaveError(providerErrorMessage(error)); showError('update the event', error) } })
    else createEvent.mutate({ orgId: org.id, ...(input as CalendarEventCreateInput) }, { onSuccess: () => { setEditor(null); toast.success('Event created.') }, onError: (error) => { setEditorSaveError(providerErrorMessage(error)); showError('create the event', error) } })
  }
