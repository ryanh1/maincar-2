...
export function CallDetail_DispositionForm({ orgId, call }: { orgId: string; call: CallDetail }) {
  ...
  if (!dispositionsQuery.data?.dispositions.length) return <section className="border border-border p-3"><h2 className="text-sm font-semibold">Call outcome</h2><p className="mt-1 text-sm text-text-muted">Add a disposition before logging this call.</p><Button asChild className="mt-3" size="sm" variant="secondary"><Link to="/settings/dispositions">Manage dispositions</Link></Button></section>
