...
describe('useGetAccountTimeline', () => {
  ...
  it('sends an explicit range and filters with the account scope', async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(jsonFetch).toHaveBeenCalledWith(
      '/api/orgs/org-1/account-timeline?rootType=company&rootId=company-1&occurredFrom=2026-08-01T00%3A00%3A00.000Z&occurredTo=2026-09-01T00%3A00%3A00.000Z&limit=25&sourceType=call&direction=outbound&personId=person-1&dealId=deal-1',
    )
  })

  it('uses the server cursor for the next page without changing the cached scope', async () => {
    jsonFetch
    ...
    const { client, result } = renderGetAccountTimeline('org-1', companyRoot)
    ...
    expect(jsonFetch).toHaveBeenLastCalledWith(
      '/api/orgs/org-1/account-timeline?rootType=company&rootId=company-1&cursor=cursor-1',
    )
    expect(client.getQueryData(queryKeys.accountTimeline.list('org-1', companyRoot, {}))).toBeDefined()
  })
